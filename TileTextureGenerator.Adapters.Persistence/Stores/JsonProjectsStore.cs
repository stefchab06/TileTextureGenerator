...
public sealed class JsonProjectsStore : IProjectsStore
{
    ...
    /// <inheritdoc />
    public async Task ArchiveAsync(string projectName)
    {
        ArgumentNullException.ThrowIfNull(projectName);
        ...
        if (!await ExistsAsync(projectName))
            throw new InvalidOperationException($"Project '{projectName}' does not exist and cannot be archived.");
        ...
        // Load project to get current state and update Status
        ProjectBase? project = await LoadAsync(projectName);
        if (project == null)
            throw new InvalidOperationException($"Failed to load project '{projectName}' for archiving.");

        // Update project metadata
        project.Status = Core.Enums.ProjectStatus.Archived;
