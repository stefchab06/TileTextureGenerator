...
public class JSonProjectStoreTests
{
    ...
    #region ITransformationStore.SaveAsync Tests
    ...
    public async Task SaveAsync_SimpleTransformation_SavesCorrectly()
    {
        ...
        var transformation = new HorizontalFloorTransformation(_store);
        var transformationId = Guid.NewGuid();
        transformation.Initialize(project, transformationId);
        transformation.TileShape = TileShape.HalfHorizontal;

        // Act
        await ((ITransformationStore)_store).SaveAsync(transformation);
